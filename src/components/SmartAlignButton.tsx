...
interface SmartAlignButtonProps {
  state: AlignState;
  onTrigger: () => void;
  correlation?: number;
...
export default function SmartAlignButton({
  ...
  useEffect(() => {
    const rpm = state === 'scanning' ? 180 : 45;
      ...
      >
        ...
        >
          ...
          <g ref={orbitalRef} style={{ transformOrigin: `${cx}px ${cy}px` }}>
            {dots.map((d, i) => {
              const brightness = state === 'scanning' ? (i % 3 === 0 ? 0.9 : 0.3) : (i % 4 === 0 ? 0.6 : 0.18);
              return (
                <circle
                  key={i}
                  cx={cx + d.x}
        ...
        <button
          ...
          style={{
            ...
            height: 100,
            borderRadius: '50%',
            border: `1px solid ${state === 'locked' ? 'rgba(255,255,255,0.6)' : 'rgba(255,255,255,0.22)'}`,
            background:
              state === 'locked'
        ...
        >
          ...
          {state === 'locked' && (
            <>
              <div style={{ fontSize: 7.5, color: 'rgba(255,255,255,0.5)', letterSpacing: '0.12em' }}>
                PHASE LOCKED
              </div>
              <div
                style={{
              ...
              <div style={{ fontSize: 7, color: 'rgba(255,255,255,0.4)', letterSpacing: '0.1em' }}>
                CORR
              </div>
            </>
          )}
